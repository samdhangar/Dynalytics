...
foreach ($transactionVaultBuys as $transactionVaultBuy):
    ...
    $exportArr = array(
        ...
        (isset($transactionVaultBuy['TransactionVaultBuy']['trans_datetime']) ? showdate($transactionVaultBuy['TransactionVaultBuy']['trans_datetime']) : ''),
    );
$this->CSV->addRow($exportArr);
endforeach;

$filename = getReportName('TransactionVaultBuys');
