<?php
$this->assign('pagetitle', __('Vault Buy Report'));
$this->Custom->addCrumb(__('Analytics'));
$this->start('top_links');
echo $this->Html->link('<span>' . __(getReportFilter($this->Session->read('Report.ValutBuyReport'))) . '</span>', 'javascript:void(0)', array('data-toggle' => 'tooltip', 'title' => __('Select Date'), 'icon' => 'icon-calendar', 'class' => 'btn btn-primary btn-sm daterange pull-right marginleft', 'escape' => false));
echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'valut_buy'), array('title' => __('Export CSV'), 'icon' => 'icon-download', 'class' => 'btn btn-primary btn-sm pull-right', 'escape' => false));
$this->end();
...
<div class="panel panel-flat" style="float:left;width:100%;">
    ...
    <div class="col-md-12 col-sm-12 form-group row">
            <div class="box box-primary">
                <div class="box-body">
                    <?php
                    echo $this->Form->create('Analytic', array('url'=>array('controller'=>'analytics','action'=>'valut_buy'),'id' => 'analyticForm', 'inputDefaults' => array('class' => 'form-control', 'div' => array('class' => 'form-group'))));
                    if(!isCompany()):
                    ...
                    echo $this->Html->link(__('Reset Filter'), array('controller' => 'analytics', 'action' => 'valut_buy', 'all'), array('title' => __('Reset Filter'), 'icon' => 'fa-refresh', 'class' => 'btn btn-default marginleft', 'escape' => false));
                    echo $this->Form->end();
...
<div class="panel panel-flat"  style="float:left;width:100%;">
    <div class="panel-heading">
        <h5 class="panel-title">Vault Buy Data</h5>
    </div>
    ...
    <div class="table-responsive">
        <?php echo $this->element('reports/valut_buy', array('activity' => $valutBuys, 'companyDetail' => $companyDetail,'type'=>'vault'));?>
    </div>
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        var startDate = moment('<?php echo $this->Session->read('Report.ValutBuyReport.start_date') ?>', 'YYYY-MM-DD');
        var endDate = moment('<?php echo $this->Session->read('Report.ValutBuyReport.end_date') ?>', 'YYYY-MM-DD');

        addDateRange(startDate, endDate, "analytics/valut_buy", 'lineChart');
