...
if (in_array("byHour", $uri_get) && in_array("hourbranch",$uri_get)) {
    echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_hour', 'byHour','hourbranch'), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
} elseif (in_array("byHour", $uri_get) && in_array("stationhour",$uri_get)) {
    echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_hour', 'byHour','stationhour'), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
}elseif (in_array("byHour", $uri_get) && in_array("userhour",$uri_get)) {
    echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_hour', 'byHour','userhour'), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
}
foreach ($tellerNames_Arr as $key => $value) {
    if (in_array("byHour", $uri_get) && in_array("userdetails",$uri_get) && in_array($value,$uri_get)) {
        echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_hour', 'byHour','userdetails',$value), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
    }
}
$this->end();
                    ...
                    <thead>
                        <tr>
                            ...
                            <?php if (!$this->Session->check('Auth.User.BranchDetail.id')) { ?>
                                ...
                                <?php if (in_array("stationhour", $uri_get) || in_array("userhour", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                    <th>
                                        <?php
                                        echo ( __('Station Name'));
                                        ?>
                                    </th>
                                <?php } ?>
                                <?php if (in_array("userhour", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                    <th>
                                        <?php
                                        echo ( __('User Name'));
                                        ?>
                        ...
                        <?php foreach ($allData as $key => $all_transactions) : ?>
                            <tr>
                                ...
                                <?php if (!$this->Session->check('Auth.User.BranchDetail.id')) { ?>
                                    ...
                                    <td>
                                        <?php
                                        echo isset($all_transactions['m']['branch_id']) ? $this->Html->link($branches[$all_transactions['m']['branch_id']], array('controller' => 'users', 'action' => 'dashboard_hour', 'byHour', 'stationhour', '?' => ['branch_id' => $all_transactions['m']['branch_id']])) : '-';
                                        ?>
                                    </td>
                                    <?php if (in_array("stationhour", $uri_get) || in_array("userhour", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                        <td>
                                            <?php
                                            echo isset($all_transactions['m']['station']) ? $this->Html->link($temp_station[$all_transactions['m']['station']], array('controller' => 'users', 'action' => 'dashboard_hour', 'byHour', 'userhour','?' => ['station_id' => $all_transactions['m']['station']])) : '-';
                                            ?>
                                        </td>
                                    <?php } ?>
                                    <?php if (in_array("userhour", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                        <td>
                                            <?php
                                                echo isset($all_transactions['m']['teller_name']) ? $this->Html->link($all_transactions['m']['teller_name'], array('controller' => 'users', 'action' => 'dashboard_hour', 'byHour', 'userdetails',$all_transactions['m']['teller_name'])) : '-';
                                            ?>
                                        </td>
                                    <?php } ?>
                                <?php } ?>
                                <td>
                                    ...
                                    echo isset($all_transactions['TransactionDetail']['trans_datetime']) ? date("M-Y", strtotime($all_transactions['TransactionDetail']['trans_datetime'])) : ''; 
                                   }elseif (in_array("yearly", $uri_get)) {
                                    echo isset($all_transactions['TransactionDetail']['trans_datetime']) ? date("Y", strtotime($all_transactions['TransactionDetail']['trans_datetime'])) : ''; 
                                   }elseif (in_array("byHour", $uri_get)) {
                                    $date=date_create($all_transactions['m']['HOUR'].":00:00");
                                    $time = date_format($date,"g A");
                                    echo isset($all_transactions['m']['HOUR']) ? $time: '';

                                    // echo isset($all_transactions['m']['HOUR']) ? sprintf("%02d", $all_transactions['m']['HOUR']) : '';
                                    }
                                    ?>
                                </td>
                               <td>
                                ...
                                <td>
                                    <?php  if (in_array("hourbranch", $uri_get)) {
                                        echo $this->Html->link(isset($all_transactions[0]['sum(m.COUNT)'])  ? $all_transactions[0]['sum(m.COUNT)'] : 0,array('controller' => 'users', 'action' => 'transaction_detail',$all_transactions['m']['company_id'],$all_transactions['m']['regiones'],'hour',sprintf("%02d", $all_transactions['m']['HOUR']),$all_transactions['m']['branch_id']));
                                    }elseif (in_array("stationhour", $uri_get)) {
                                        echo $this->Html->link(isset($all_transactions[0]['sum(m.COUNT)'])  ? $all_transactions[0]['sum(m.COUNT)'] : 0,array('controller' => 'users', 'action' => 'transaction_detail',$all_transactions['m']['company_id'],$all_transactions['m']['regiones'],'hour',sprintf("%02d", $all_transactions['m']['HOUR']),$all_transactions['m']['branch_id'],$all_transactions['m']['station']));
                                    }elseif (in_array("userhour", $uri_get)) {
                                        echo $this->Html->link(isset($all_transactions[0]['sum(m.COUNT)'])  ? $all_transactions[0]['sum(m.COUNT)'] : 0,array('controller' => 'users', 'action' => 'transaction_detail',$all_transactions['m']['company_id'],$all_transactions['m']['regiones'],'hour',sprintf("%02d", $all_transactions['m']['HOUR']),$all_transactions['m']['branch_id'],$all_transactions['m']['station'],$all_transactions['m']['teller_name']));
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        ...
        // $(".pagination").children.find('li .active')(function(e) {
