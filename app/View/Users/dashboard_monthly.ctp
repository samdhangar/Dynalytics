...
if (in_array("monthly", $uri_get) && in_array("hourmonthly", $uri_get)) {
    echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_monthly', 'monthly', 'hourmonthly'), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
} elseif (in_array("monthly", $uri_get) && in_array("stationmonth", $uri_get)) {
    echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_monthly', 'monthly', 'stationmonth'), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
} elseif (in_array("monthly", $uri_get) && in_array("usermonth", $uri_get)) {
    echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_monthly', 'monthly', 'usermonth'), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
}
foreach ($tellerNames_Arr as $key => $value) {
    if (in_array("monthly", $uri_get) && in_array("userdetails", $uri_get) && in_array($value, $uri_get)) {
        echo $this->Html->link(__('Export CSV'), array('controller' => 'export', 'action' => 'dashboard_monthly', 'monthly', 'userdetails', $value), array('title' => __('Export CSV'), 'icon' => ' icon-download', 'class' => 'btn btn-primary btn-sm pull-right marginleft', 'id' => 'exportBtn', 'escape' => false));
    }
}
$this->end();
                    ...
                    <thead>
                        <tr>
                            ...
                            <?php if (!$this->Session->check('Auth.User.BranchDetail.id')) { ?>
                                ...
                                <?php if (in_array("stationmonth", $uri_get) || in_array("usermonth", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                    <th>
                                        <?php
                                ...
                                <?php } ?>
                                <?php if (in_array("usermonth", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                    <th>
                                        <?php
                                        echo (__('User Name'));
                                        ?>
                        ...
                        <?php foreach ($allData as $key => $all_transactions) : ?>
                            <tr>
                                ...
                                <?php if (!$this->Session->check('Auth.User.BranchDetail.id')) { ?>
                                    ...
                                    <td>
                                        <?php
                                        echo isset($all_transactions['m']['branch_id']) ? $this->Html->link($branches[$all_transactions['m']['branch_id']], array('controller' => 'users', 'action' => 'dashboard_monthly', 'monthly', 'stationmonth','?' => ['branch_id' => $all_transactions['m']['branch_id']])) : '-';
                                        ?>
                                    </td>
                                    <?php if (in_array("stationmonth", $uri_get) || in_array("usermonth", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                        <td>
                                            <?php
                                            echo isset($all_transactions['m']['station']) ? $this->Html->link($temp_station[$all_transactions['m']['station']], array('controller' => 'users', 'action' => 'dashboard_monthly', 'monthly', 'usermonth','?' => ['station_id' => $all_transactions['m']['station']])) : '-';
                                            ?>
                                        </td>
                                    <?php } ?>
                                    <?php if (in_array("usermonth", $uri_get) || in_array("userdetails", $uri_get)) { ?>
                                        <td>
                                            <?php
                                            echo isset($all_transactions['m']['teller_name']) ? $this->Html->link($all_transactions['m']['teller_name'], array('controller' => 'users', 'action' => 'dashboard_monthly', 'monthly', 'userdetails', $all_transactions['m']['teller_name'])) : '-';
                                            ?>
                                        </td>
                                    <?php } ?>
                                <?php } ?>
                                ...
                                <td>
                                    <?php if (in_array("hourmonthly", $uri_get)) {
                                        echo $this->Html->link(isset($all_transactions[0]['sum(m.COUNT)'])  ? $all_transactions[0]['sum(m.COUNT)'] : 0, array('controller' => 'users', 'action' => 'transaction_detail', $all_transactions['m']['company_id'], $all_transactions['m']['regiones'], 'monthly', $all_transactions['m']['MONTHNAME'], $all_transactions['m']['branch_id']));
                                    } elseif (in_array("stationmonth", $uri_get)) {
                                        echo $this->Html->link(isset($all_transactions[0]['sum(m.COUNT)'])  ? $all_transactions[0]['sum(m.COUNT)'] : 0, array('controller' => 'users', 'action' => 'transaction_detail', $all_transactions['m']['company_id'], $all_transactions['m']['regiones'], 'monthly', $all_transactions['m']['MONTHNAME'], $all_transactions['m']['branch_id'], $all_transactions['m']['station']));
                                    } elseif (in_array("usermonth", $uri_get)) {
                                        echo $this->Html->link(isset($all_transactions[0]['sum(m.COUNT)'])  ? $all_transactions[0]['sum(m.COUNT)'] : 0, array('controller' => 'users', 'action' => 'transaction_detail', $all_transactions['m']['company_id'], $all_transactions['m']['regiones'], 'monthly', $all_transactions['m']['MONTHNAME'], $all_transactions['m']['branch_id'], $all_transactions['m']['station'], $all_transactions['m']['teller_name']));
