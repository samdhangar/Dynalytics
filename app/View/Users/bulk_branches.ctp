...
$this->assign('pagetitle', __('Add Bulk Branches'));
$count_amount_flag = 0;
if (!empty($activityReportId)) {
    ...
    $this->Custom->addCrumb(__('Add Bulk Branches'));
} else {
    $this->Custom->addCrumb(__('Add Bulk Branches'));
}
$this->start('top_links');
    echo $this->Html->link(__('Download Sample CSV File'), array('action' => 'downloadSamplefile',base64_encode("Branch_Sample")), array('class' => 'btn btn-sm btn-success btn', 'full_base' => true));
$this->end();
?>
<div class="row">
...
<script>
    ...
    jQuery(document).ready(function() {
        jQuery('#confirmbtn').on('click', function() {
            ...
            jQuery.ajax({
                type: "POST",
                url: BaseUrl + "/users/bulk_branches/",
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
                    ...
                    if(response1.status == 'fail'){
                        ...
                        $(".notification-message2").css("display", "block");
                        removealertmessage();
                        window.location = BaseUrl + 'users/bulk_station';
                    }
                },
                error: function(e) {
                    loader('hide');
