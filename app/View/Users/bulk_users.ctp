...
$this->assign('pagetitle', __('Add Bulk Users'));
$count_amount_flag = 0;
if (!empty($activityReportId)) {
    ...
    $this->Custom->addCrumb(__('Add Bulk Users'));
} else {
    $this->Custom->addCrumb(__('Add Bulk Users'));
}
$this->start('top_links');
    echo $this->Html->link(__('Download Sample CSV File'), array('action' => 'downloadSamplefile',base64_encode("Sample_user_company_bulk")), array('class' => 'btn btn-sm btn-success btn', 'full_base' => true));
$this->end();
?>
<div class="row">
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if(response1.status == 'fail'){
                        ...
                        $(".notification-message2").css("display", "block");
                        removealertmessage();
                        window.location = BaseUrl + 'users/bulk_regions';
                    }
                },
                error: function(e) {
                    loader('hide');
