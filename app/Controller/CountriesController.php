...
class CountriesController extends AppController
{
    ...
    {
        $conditions = array('NOT' => array('Country.status' => 'deleted'));
        if ($all == "all") {
            $this->Session->write('CountrySearch', '');
        }
    ...
    public function add()
    {
       // echo "<pre>";
        ...
        if ($this->request->is('post')) {
            $this->Country->create();
            if (empty($this->request->data['Country']['user_id'])) {
//                $this->request->data['Country']['user_id'] = $this->Session->read('Auth.User.id');
                $this->request->data['Country']['user_id'] = $sessionData['id'];
            }            
