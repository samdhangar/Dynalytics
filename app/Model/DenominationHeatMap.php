...
class DenominationHeatMap extends AppModel
{
      public $useTable = 'denomination_heat_maps'; // This model uses a database table 'exmp'
    /**
     * Display field
     *
     * @var string
...
//The Associations below have been created with all possible keys, those that are not needed can be removed
  ...
  public $belongsTo = array(
   ...
   /* function getRegionList($conditions)
    ...
    function deleteAddressHierarchy($id = null, $deleteFor = 'denomination_heat_maps')
    {
        $sessionData = getMySessionData();
        $deleteArray = array('denomination_heat_maps', 'company_branches');
        if (empty($id)) {
            return false;
        }
        $fields = array(
...
//            $conditions[ucfirst($deleteFor) . '.user_id'] = CakeSession::read('Auth.User.id');
        ...
        foreach ($deleteArray as $className) {
            if ($deleteStart == 1) {
                $conditions[ucfirst($className) . '.denomination_heat_maps'] = $id;
