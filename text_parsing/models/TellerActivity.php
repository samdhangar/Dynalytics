...
Hook::sub("subscription", function($line){
	...
	/*-------add new code on 21-10-2016---------*/
	else if(preg_match_all("/([152]0{0,2})[ ]+([0-9]+)[ ]+[0-9\.]+/", strtolower($line), $matches))
	{
		
		TellerActivityRecord::instance()->_counter = 0;
		...
		{
			$k = "d_denom_$key";
			TellerActivityRecord::instance()->$k = intVal($value);
