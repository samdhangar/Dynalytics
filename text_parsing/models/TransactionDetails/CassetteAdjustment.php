...
$_i = CassetteAdjustment::getInstance();

Hook::sub("subscription", array($_i, "index"));

class CassetteAdjustment
{
	...
	public function index($line)
	{
		...
		if(preg_match_all("/([A-Za-z]+(( |-)([A-Za-z]+|[0-9]))*) #?[ ]+(((\+|\-)?[0-9\/\.:]+( pm| am)?)|\n)/", strtolower($this->line), $matches))
		{
			...
			/*    *** Total Net Added Section ***     */
			if($key == "total net added")
			{
				$this->transaction->non_cash_total = floatval($value);
			}

			/*    *** Total Net Removed Section ***     */
			if($key == "total net removed")
			{
				$this->transaction->balance_due = floatval($value);
			}
	...
	{
		$matches = array();
		return preg_match_all("/^(\(Cassette Adjustment\))[ ]+/", $this->line, $matches);
	}
